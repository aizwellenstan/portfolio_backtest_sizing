...
for risk_pct in [i / 1000 for i in range(1, 300)]:
    for min_risk in range(10, 300, 1):
        ...
        for ppc in df['ProfitPerContract']:
            if ppc == 0:
                ...
                risk_dollars = abs(ppc) * dynamic_contracts
                if risk_dollars < min_risk:
                    contracts = math.ceil(min_risk / abs(ppc))
                else:
            ...
            if balance > peak_balance:
                delta = balance - peak_balance
                trailing_stop += delta
                peak_balance = balance

            if balance < trailing_stop:
                valid = False
                break

        if not valid or not trade_profits:
            continue
        ...
        # ✅ Prefer higher risk_pct if expectancy is equal
        if expectancy > best_score or (expectancy == best_score and risk_pct > best_combo[0]):
            best_score = expectancy
            best_combo = (risk_pct, min_risk)
...
# === Output ===
print(f"Expectancy Optimization with Trailing Stop and Minimum Risk")
print(f"Optimal risk_pct: {best_combo[0]:.3f}, min_risk: ${best_combo[1]}")
print(f"Expectancy: ${best_score:.2f} per trade")
print(f"Final Profit: ${final_profit:.2f}")
print(f"Max Drawdown: ${mdd:.2f}")
print("\nTrailing Stop History:")
for row in trailing_stops:
    print(f"Trade {row['Trade']:>3}: Balance = ${row['Balance']:>8.2f}, Trailing Stop = ${row['TrailingStop']:>8.2f}, New Peak = {row['NewPeak']}, MDD = ${row['MDD']:>6.2f}")
